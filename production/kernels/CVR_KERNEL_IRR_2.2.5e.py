# ==========================================================================================
# CVR KERNEL G3_2.2.5e_IRR (Consolidated: ENRICHMENT + SCENARIO + IRR)
# ==========================================================================================
...
# - G3_2.2.5e IRR extension (TF methodology, CR layering, fork generation)
#
# T2 EXECUTION: Execute via CLI (Pattern 6):
#   python3 CVR_KERNEL_IRR_2.2.5e.py --a13 <A13.json> --inputs <INPUTS.json> --output <A14.json>
# ==========================================================================================
...
# ==========================================================================================
# CLI INTERFACE (Pattern 6 Compliance)
# ==========================================================================================
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="CVR Kernel IRR 2.2.5e - Execute IRR workflow via CLI"
    )
    parser.add_argument(
        "--a13",
        required=True,
        help="Path to A.13_RESOLUTION_TIMELINE JSON file"
    )
    parser.add_argument(
        "--inputs",
        required=True,
        help="Path to IRR_INPUTS JSON file (market data, valuation anchor, scenarios)"
    )
    parser.add_argument(
        "--output",
        required=True,
        help="Output path for A.14_IRR_ANALYSIS JSON file"
    )

    args = parser.parse_args()

    # Load A.13 (resolution timeline with rho estimates)
    print(f"Loading A.13 from: {args.a13}")
    with open(args.a13, 'r') as f:
        a13_resolution_timeline = json.load(f)

    # Load IRR inputs (market data, fundamentals, scenarios, valuation anchor)
    print(f"Loading IRR inputs from: {args.inputs}")
    with open(args.inputs, 'r') as f:
        irr_inputs = json.load(f)

    # Extract rho estimates from A.13
    rho_estimates = {}
    for res in a13_resolution_timeline.get('resolution_estimates', []):
        scenario_id = res.get('scenario_id')
        rho = res.get('resolution_percentage_rho', 0.30)
        rho_estimates[scenario_id] = rho

    # Build CVR State 4 bundle structure expected by execute_irr_workflow
    # The kernel expects specific artifact structures
    cvr_state4_bundle = {
        'A.2_ANALYTIC_KG': {
        ...
        'A.10_SCENARIO_MODEL_OUTPUT': {
            'scenario_definitions': irr_inputs.get('scenarios_finalized', []),
            'sse_results': {
                'jpd': []  # Will be computed by kernel if needed
            }
        },
        'A.12_INTEGRATION_TRACE': {
            'metadata': {
                'valuation_date': irr_inputs.get('valuation_date', ''),
            },
            'pipeline_fit': {
                'grade': irr_inputs.get('pipeline_fit_grade', 'B'),
                'caveat': irr_inputs.get('pipeline_fit_caveat', None),
            },
            'state_4_synthesis': {
    ...
    # Extract convergence rate from A.13
    cr_assessment = a13_resolution_timeline.get('convergence_rate_assessment', {})
    convergence_rate = cr_assessment.get('cr_final', 0.20)
    ...
    try:
        ...
        # Write A.14 output
        print(f"Writing A.14 to: {args.output}")
        with open(args.output, 'w') as f:
            json.dump(a14_result, f, indent=2, default=str)

        print(f"SUCCESS: A.14_IRR_ANALYSIS written to {args.output}")
        sys.exit(0)

    except Exception as e:
        print(f"ERROR: IRR workflow failed: {e}")
        import traceback
        traceback.print_exc()
        sys.exit(1)