#!/usr/bin/env python3
"""
CLI wrapper for CVR_KERNEL_ENRICH.

Usage:
    python3 run_enrich_kernel.py \
...
It produces A.7 (Valuation Summary) - same as BASE but with enriched inputs.
"""
...
def main():
    parser = argparse.ArgumentParser(description='Run ENRICH CVR Kernel')
    ...
    kernel_file = os.path.join(KERNELS_DIR, f"CVR_KERNEL_ENRICH_{args.kernel_version}.py")
    ...
    # Execute kernel
    print(f"\n{'='*60}")
    print(f"Executing ENRICH CVR Kernel v{args.kernel_version}")
    print(f"{'='*60}\n")
