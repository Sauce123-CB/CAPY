...
def calculate_apv(forecast_df, dr, kg, unit_multiplier=1):
    ...
    # 4. Determine Terminal Growth (g) - PATCH 2.2.3e: from topline, not NOPAT
    ...
    # 9. Calculate IVPS (PATCH 2.2.3e: apply unit multiplier for per-share calculation)
...
# ==========================================================================================
# SCENARIO EXTENSION MODULE (Inherited from SCENARIO 2.2.1e for INTEGRATION)
# ==========================================================================================
# 
# The following functions extend the base kernel for SCENARIO/INTEGRATION execution:
# - apply_gim_overlay(): Apply parametric interventions to GIM
# - apply_structural_modifications(): Apply structural DAG modifications
# - execute_scenario_intervention(): Execute full scenario intervention
# - calculate_sse_jpd(): Structured State Enumeration with constraints
# - calculate_probability_sensitivity(): Scenario probability sensitivities
# - analyze_distribution(): Distribution metrics computation
# - generate_scenario_summary(): Summary artifact generation
# - execute_full_scenario_analysis(): Complete SCENARIO/INTEGRATION pipeline
#
# ==========================================================================================
...
def execute_scenario_intervention(kg, dag, gim, dr_trace, intervention_def, dr_override=None, unit_multiplier=1):
    ...
    # 10. Extract Y1 fundamentals from intervened forecast for IRR stage (Patch 1.1)
    # These fundamentals reflect the scenario-specific trajectory
    fundamentals_y1_intervened = {}
    if forecast_df is not None and len(forecast_df) > 0:
        # Y1 is typically index 1 (Y0=0, Y1=1, etc.) but fallback to first row if needed
        y1_idx = 1 if len(forecast_df) > 1 else 0
        
        # Extract key metrics with safe column access
        if 'Revenue' in forecast_df.columns:
            fundamentals_y1_intervened['revenue'] = float(forecast_df['Revenue'].iloc[y1_idx])
        if 'EBITDA' in forecast_df.columns:
            fundamentals_y1_intervened['ebitda'] = float(forecast_df['EBITDA'].iloc[y1_idx])
        if 'EBIT' in forecast_df.columns:
            fundamentals_y1_intervened['ebit'] = float(forecast_df['EBIT'].iloc[y1_idx])
            # Calculate EBIT margin if revenue available
            if 'revenue' in fundamentals_y1_intervened and fundamentals_y1_intervened['revenue'] > EPSILON:
                fundamentals_y1_intervened['ebit_margin'] = (
                    fundamentals_y1_intervened['ebit'] / fundamentals_y1_intervened['revenue']
                )
        if 'NOPAT' in forecast_df.columns:
            fundamentals_y1_intervened['nopat'] = float(forecast_df['NOPAT'].iloc[y1_idx])
        if 'FCF_Unlevered' in forecast_df.columns:
            fundamentals_y1_intervened['fcf_unlevered'] = float(forecast_df['FCF_Unlevered'].iloc[y1_idx])
        elif 'FCF' in forecast_df.columns:
            fundamentals_y1_intervened['fcf_unlevered'] = float(forecast_df['FCF'].iloc[y1_idx])
    ...
    return {
        ...
        'fundamentals_y1_intervened': fundamentals_y1_intervened  # Patch 1.1: IRR stage input
    }
# ==========================================================================================
...
def execute_full_scenario_analysis(kg, dag, gim, dr_trace, base_ivps, scenario_definitions, constraints, unit_multiplier=1):
    ...
    for scenario_def in scenario_definitions:
        ...
        scenario_results.append({
            ...
            'fundamentals_y1_intervened': intervention_result.get('fundamentals_y1_intervened', {})  # Patch 1.1
        })
