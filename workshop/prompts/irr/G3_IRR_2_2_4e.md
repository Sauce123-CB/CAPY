G3 IRR 2.2.4e: Expected Return Analysis
________________


I. MISSION AND OBJECTIVES
Mission: Execute the IRR stage (G3_2.2.4eIRR) of the CAPY Pipeline for {Company Name} ({EXCHANGE:TICKER}) as of {DATE}.
...
A. Mandatory Inputs
The IRR stage receives the **CVR State 4 Bundle** from INTEGRATION 2.2.2e—a consolidated JSON object containing all finalized artifacts. The bundle structure (defined in INTEGRATION Section II.F) provides:

**Compatibility:** This stage consumes INT 2.2.2e T2 output. The 
bundle_metadata.schema_version should be `G3_2.2.2eI_BUNDLE`.
...
- This prompt (G3_IRR_2.2.4e.md)
- INT T2 Output ({TICKER}_INT2.2.2eO_T2_{YYYYMMDD}.md) containing CVR State 4 Bundle
- CVR_KERNEL_IRR_2.2.4e.py (context only — DO NOT EXECUTE)
...
- Filename: {TICKER}_IRR2.2.4eO_T1_{YYYYMMDD}.md
...
**Attachments Required:**
- This prompt (G3_IRR_2.2.4e.md)
- T1 Output ({TICKER}_IRR2.2.4eO_T1_{YYYYMMDD}.md)
- CVR State 4 Bundle (fresh context)
- CVR_KERNEL_IRR_2.2.4e.py

**Scope:**
- Load kernel
- Call execute_irr_workflow() with A.13 inputs
- Format A.14 output

**Output:**
- A.14_IRR_ANALYSIS artifact
- Executive summary narrative
- Filename: {TICKER}_IRR2.2.4eO_T2_{YYYYMMDD}.md

**Critical:** T2 performs NO reasoning. All analytical judgment is locked in A.13.


B. The CVR Kernel Mandate
The CVR Kernel (G3_2.2.4e_IRR) is the sole authorized execution engine for all IRR calculations.
1. Loading & Execution (Two-Shot File Delivery):
   * Turn 1: CVR_KERNEL_IRR_2.2.4e.py is attached for contextual understanding 
     only (function signatures, TF methodology, CR integration, fork generation logic). 
     DO NOT execute kernel code in T1.
   * Turn 2: Load CVR_KERNEL_IRR_2.2.4e.py directly into the execution environment.
   * Action: In T2, import and call functions directly:
     `from CVR_KERNEL_IRR_2_2_4e import execute_irr_workflow`
Required Functions:
...
Emission Format: JSON, validated against schemas in Appendix A.
—------------------------------
...

{
  ...
  "version_control": {
    ...
    "pipeline_stage": "IRR G3_2.2.4e",
    "execution_model": "Two-Shot (T1=Analytical, T2=Computational)",
    "int_compatibility": "G3INT 2.2.2e"
  },
...
—-----------
APPENDIX C: CVR KERNEL G3_2.2.4e_IRR
