...
APPENDIX C: CVR Kernel

---

## CVR KERNEL G3_2.2.2e_INT (Consolidated)

**File:** CVR_KERNEL_INT_2.2.2e.py

**Structure:**
- Base: ENRICHMENT 2.2.1e core functions (DSL, SCM, APV engines)
- Extension: SCENARIO 2.2.1e module (intervention, SSE, distribution analysis)
- INTEGRATION: No additional computational functions; adjudication handled at prompt level

**Key Functions for INTEGRATION:**
- `execute_cvr_workflow()`: Base case recalculation when GIM modified
- `execute_scenario_intervention()`: Scenario magnitude recalculation
- `calculate_sse_jpd()`: SSE integration with constraints
- `execute_full_scenario_analysis()`: Complete recalculation pipeline

**Usage (T2):**
```python
from CVR_KERNEL_INT_2_2_2e import (
    execute_cvr_workflow,
    execute_scenario_intervention,
    calculate_sse_jpd,
    execute_full_scenario_analysis
)
```
...
\# CVR KERNEL G3_2.2.2e_INT (INTEGRATION Implementation - Consolidated)

\# Consolidated: SCENARIO 2.2.1e Base + INTEGRATION Extensions
...
start_value = float(params.get(\'start_value\', y0_value if y0_value is
not None else 0))
...
\# Linear interpolation from start to end over fade_years
...
raise ValueError(\"CAGR_INTERP requires a valid y0_value from
ANALYTIC_KG.\")
...
\# Linear interpolation of growth rates over interp_years
...
\# Logistic function: value(t) = start + (saturation - start) / (1 +
exp(-k\*(t - t0)))
...
forecast\[i\] = start_value + (saturation_value - start_value) \*
logistic_factor
...
raise ValueError(\"MULTI_STAGE_FADE requires at least one stage
definition.\")
...
current_value = float(params.get(\'start_value\', y0_value if y0_value
is not None else 0))
...
stage_values = np.linspace(current_value, end_value, stage_years +
1)\[1:\]
...
raise ValueError(f\"Unknown DSL mode: {mode}. Supported: STATIC,
LINEAR_FADE, CAGR_INTERP, S_CURVE, MULTI_STAGE_FADE,
EXPLICIT_SCHEDULE\")
...
Ensures only intra-timestep dependencies (GET calls) are considered for
ordering.
...
raise RuntimeError(f\"Cycle detected in DAG. Check nodes for circular
dependencies (use PREV for lagged): {cycle_nodes}\")
...
Validates that all Y0_data metrics are accounted for in the DAG (P5
Doctrine).
...
logger.warning(f\"P5 Warning: Y0_data metrics not explicitly
dispositioned in DAG: {missing_metrics}\")
...
fdso = share_data.get(\'shares_out_diluted_tsm\') or
share_data.get(\'FDSO\')

\# Capital structure for equity bridge

capital_structure = kg.get(\'capital_structure\',
{}).get(\'net_debt_y0\', {})
...
raise ValueError(\"FDSO/shares_out_diluted_tsm must be provided and
positive in ANALYTIC_KG.\")
...
terminal_g_estimate = np.mean(nopat_growth_rates\[-3:\]) \# Average of
last 3 years
...
logger.warning(f\"Terminal growth ({terminal_g:.4f}) \>= DR ({dr:.4f}).
Adjusting g to 99% of DR.\")
...
logger.warning(\"Terminal Reinvestment Rate is negative. Assuming
g=0.\")
...
raise RuntimeError(f\"APV denominator (DR - g) is non-positive. DR={dr},
g={terminal_g}\")
...
\'FCF_Unlevered\', \'Invested_Capital\'\]
...
Runs sensitivity analysis (Tornado chart) by modifying GIM assumptions
or DR.
...
params\[\'end_value\'\] = apply_change(params\[\'end_value\'\],
pct_change)
...
apply_change(params\[\'saturation_value\'\], pct_change)
...
stage\[\'end_value\'\] = apply_change(stage\[\'end_value\'\],
pct_change)
...
logger.warning(f\"Sensitivity analysis error for {driver} ({direction}):
{e}\")
...
This artifact provides a condensed summary of the valuation for
downstream stages,

including the IRR model which requires near-term trajectory data.

Key outputs include:

\- IVPS summary (base case valuation, DR, terminal values)
...
\- Forecast trajectory checkpoints: Y0, Y1, Y2, Y3, Y5, Y10, Y20

PATCHED (G3_2.2.2eI): Added Y1, Y2, Y3 checkpoints for IRR T+1 modeling.

The IRR stage requires explicit near-term fundamentals to avoid
interpolation

errors when the SCM models non-linear near-term dynamics (S-curves,
contract

cliffs, capital cycle dips, etc.).

Checkpoint purposes:

\- Y0: Historical baseline (from A.2_ANALYTIC_KG)

\- Y1: Critical for IRR T+1 fundamental growth calculation

\- Y2: Near-term visibility for market expectation modeling

\- Y3: Forward visibility for cohort multiple estimation

\- Y5/Y10/Y20: Medium and long-term trajectory validation
...
base_ivps = safe_float(sr.get(\'base_case_ivps\')) if sr else
ivps_summary\[\"IVPS\"\]
...
\# PATCHED: Added Y1, Y2, Y3 for IRR T+1 modeling and near-term
visibility

trajectory_checkpoints = {\"Y0\": {}, \"Y1\": {}, \"Y2\": {}, \"Y3\":
{}, \"Y5\": {}, \"Y10\": {}, \"Y20\": {}}
...
key_internal_drivers = \[\'Revenue\', \'EBIT\', \'EBITDA\', \'NOPAT\',
...
\# Y1 (Critical for IRR T+1 modeling)

if len(forecast_df) \>= 1:

trajectory_checkpoints\[\"Y1\"\]\[driver\] =
safe_float(forecast_df\[driver\].iloc\[0\])

\# Y2 (Near-term visibility for market expectations)

if len(forecast_df) \>= 2:

trajectory_checkpoints\[\"Y2\"\]\[driver\] =
safe_float(forecast_df\[driver\].iloc\[1\])

\# Y3 (Forward visibility for multiple estimation)

if len(forecast_df) \>= 3:

trajectory_checkpoints\[\"Y3\"\]\[driver\] =
safe_float(forecast_df\[driver\].iloc\[2\])

\# Y5 (Medium-term checkpoint)

if len(forecast_df) \>= 5:

trajectory_checkpoints\[\"Y5\"\]\[driver\] =
safe_float(forecast_df\[driver\].iloc\[4\])

\# Y10 (Long-term checkpoint)

if len(forecast_df) \>= 10:

trajectory_checkpoints\[\"Y10\"\]\[driver\] =
safe_float(forecast_df\[driver\].iloc\[9\])

\# Y20 (Terminal checkpoint)
...
The main API for the CVR Kernel G3_2.2.2e_INT (INTEGRATION).
...
A.5_GESTALT_IMPACT_MAP artifact (with \'GIM\' key containing driver
assumptions)
...
List of sensitivity scenario definitions: \[{\'driver\': str, \'low\':
float, \'high\': float}\]
...
raise RuntimeError(f\"Failed to parse DR from DR_DERIVATION_TRACE:
{e}\")
...
kg, dag, seq, gim, dr, valuation_results, sensitivity_scenarios,
schema_version
...
\# END OF CVR KERNEL G3_2.2.2e_INT
...
\# ==========================================================================================
\# SCENARIO MODELING AND SSE INTEGRATION FUNCTIONS
\# ==========================================================================================
...
\# The INTEGRATION stage (G3_2.2.2e_INT) uses the inherited SCENARIO functions for:

\# - Recalculation cascade (execute_cvr_workflow, execute_scenario_intervention)

\# - SSE re-integration (calculate_sse_jpd)

\# - Distribution re-analysis (analyze_distribution, generate_scenario_summary)

\#

\# No additional computational functions are required for INTEGRATION.

\# Adjudication logic, amendment tracking, and multi-SC synthesis are handled

\# at the prompt level, not within the kernel.
